...
const genHistory = (days=30,start=10000,drift=1.003) =>
  Array.from({length:days},(_,i)=>{
    ...
    return{date:date.toLocaleDateString("en",{month:"short",day:"numeric"}),value:Math.round(val)};
  });
const MOCK_VAULTS=[
  {id:1,name:"Alpha Yield Maximizer",description:"Aggressive multi-protocol yield farming",riskLevel:"aggressive",investmentGoal:"yield",status:"active",totalValueUSD:24830,initialDepositUSD:20000,autoRebalanceEnabled:true,allocations:[{protocol:"Rivera",pct:40,apy:18.4},{protocol:"Merchant Moe",pct:45,apy:22.1},{protocol:"Agni Finance",pct:15,apy:14.7}],history:genHistory(30,20000,1.004)},
  {id:2,name:"Stable Growth Fund",description:"Conservative allocation for steady returns",riskLevel:"conservative",investmentGoal:"stability",status:"active",totalValueUSD:52140,initialDepositUSD:50000,autoRebalanceEnabled:false,allocations:[{protocol:"Rivera",pct:20,apy:18.4},{protocol:"Merchant Moe",pct:10,apy:22.1},{protocol:"Agni Finance",pct:70,apy:14.7}],history:genHistory(30,50000,1.0015)},
];
const SENTIMENT={score:72,label:"Bullish",confidence:84,summary:"Strong inflows to Mantle ecosystem. Rivera TVL up 18% WoW. MNT price momentum positive.",recommendation:"Increase exposure to higher-yield protocols."};
const MOCK_RECS=[
  {id:1,vaultId:1,reason:"Merchant Moe APY spiked +3.2% due to new incentive program. Rebalancing towards higher yield while maintaining risk parameters.",current:[40,45,15],recommended:[25,60,15],projectedAPY:20.8,status:"pending"},
  {id:2,vaultId:2,reason:"Market sentiment bullish. Moderate increase in Rivera exposure captures upside while Agni Finance anchors portfolio stability.",current:[20,10,70],recommended:[30,15,55],projectedAPY:16.2,status:"pending"},
];
const TICKER_ITEMS=["MNT $0.841 +4.2%","ETH $3,241 +1.8%","Rivera APY 18.4%","Merchant Moe APY 22.1%","Agni Finance APY 14.7%","Total Mantle TVL $1.2B","MAAV AUM $2.4M +12%"];
...
// ── NEW: Grid Feature Cards Component ───────────────────────────────────────
type FeatureType = {
  title: string;
...
function FeatureCard({ feature, className, ...props }: FeatureCardProps) {
  ...
  return (
    <div 
      className={`relative overflow-hidden p-6 transition-all duration-300 hover:bg-white/5 group ${className || ''}`}
      {...props}
    >
      <div className="pointer-events-none absolute top-0 left-1/2 -mt-2 -ml-20 h-full w-full [mask-image:linear-gradient(white,transparent)]">
        <div className="from-foreground/5 to-foreground/1 absolute inset-0 bg-gradient-to-r [mask-image:radial-gradient(farthest-side_at_top,white,transparent)] opacity-100">
          <GridPattern
            ...
            y="4"
            squares={p}
            className="fill-white/5 stroke-white/20 absolute inset-0 h-full w-full mix-blend-overlay transition-opacity duration-500 group-hover:opacity-70"
          />
        </div>
      </div>
      <div className="relative z-10 mb-4 inline-flex rounded-lg bg-gradient-to-br from-purple-500/20 to-blue-500/10 p-2.5 ring-1 ring-white/10">
        <feature.icon className="text-purple-400 size-5" strokeWidth={1.5} aria-hidden />
      </div>
      <h3 className="relative z-10 mt-6 text-sm font-semibold tracking-tight text-slate-100 md:text-base">
        {feature.title}
      </h3>
      <p className="text-slate-400 relative z-10 mt-2 text-xs font-light leading-relaxed">
        {feature.description}
      </p>
    </div>
  );
...
const StatCard=({label,value,sub,accent,delay=0})=>(
  <div className="fade-up" style={{animationDelay:`${delay}s`,background:"linear-gradient(135deg,rgba(16,24,48,0.90),rgba(10,16,32,0.85))",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",border:`1px solid ${accent}30`,borderRadius:12,padding:"20px 24px",flex:1,minWidth:160,position:"relative",overflow:"hidden",boxShadow:"0 4px 24px rgba(0,0,0,0.3)"}}>
...
const Card=({children,style,glow,className})=>(
  <div className={className} style={{background:"linear-gradient(145deg,rgba(16,24,48,0.85),rgba(8,14,28,0.90))",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",border:"1px solid rgba(99,120,200,0.12)",borderRadius:14,padding:24,boxShadow:glow?"0 0 0 1px rgba(139,92,246,0.15) inset,0 8px 32px rgba(0,0,0,0.4),var(--glow-purple)":"0 4px 24px rgba(0,0,0,0.3),0 0 0 1px rgba(255,255,255,0.03) inset",transition:"all 0.3s ease",...style}}>{children}</div>
);
...
const Btn=({children,onClick,variant="primary",small,disabled,style:sx})=>{
  ...
  return <button onClick={onClick} disabled={disabled} style={{...base[variant],borderRadius:8,padding:small?"8px 16px":"12px 24px",fontSize:small?12:14,fontWeight:600,cursor:disabled?"not-allowed":"pointer",opacity:disabled?0.5:1,transition:"all 0.2s",fontFamily:"var(--sans)",whiteSpace:"nowrap",...sx}}>{children}</button>;
};
...
const Select=({label,children,...props})=>(
  <div style={{marginBottom:16}}>
    {label&&<label style={{fontSize:11,color:"var(--text2)",display:"block",marginBottom:6,fontFamily:"var(--mono)",textTransform:"uppercase",letterSpacing:"0.1em",fontWeight:500}}>{label}</label>}
    <select {...props} style={{width:"100%",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(99,120,200,0.15)",borderRadius:8,padding:"12px 16px",color:"var(--text0)",fontSize:14,outline:"none",fontFamily:"var(--sans)",appearance:"none",cursor:"pointer"}}>{children}</select>
  </div>
);
...
const Sidebar=({page,setPage,user})=>(
  <aside style={{width:240,background:"linear-gradient(180deg,rgba(4,8,16,0.95),rgba(2,4,10,0.98))",backdropFilter:"blur(32px)",WebkitBackdropFilter:"blur(32px)",borderRight:"1px solid rgba(99,120,200,0.1)",display:"flex",flexDirection:"column",height:"100vh",position:"sticky",top:0,flexShrink:0,zIndex:20}}>
    ...
    <nav style={{padding:"20px 14px",flex:1}}>
      {NAV.map(n=>(
        <button key={n} onClick={()=>setPage(n)} style={{display:"flex",alignItems:"center",gap:12,width:"100%",padding:"12px 14px",borderRadius:10,border:"none",background:page===n?"linear-gradient(90deg,rgba(139,92,246,0.15),rgba(59,130,246,0.05))":"transparent",color:page===n?"#8b5cf6":"var(--text1)",fontSize:13,fontWeight:page===n?600:500,cursor:"pointer",marginBottom:4,transition:"all 0.2s",textAlign:"left",boxShadow:page===n?"inset 0 0 0 1px rgba(139,92,246,0.2),0 2px 8px rgba(139,92,246,0.1)":"none",fontFamily:"var(--sans)"}}>
...
const AuthScreen=({onLogin})=>{
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div className="auth-in" style={{width:"100%",maxWidth:440,background:"linear-gradient(145deg,rgba(16,24,48,0.90),rgba(8,14,28,0.95))",backdropFilter:"blur(40px)",WebkitBackdropFilter:"blur(40px)",border:"1px solid rgba(139,92,246,0.25)",borderRadius:20,padding:"32px 36px",boxShadow:"0 0 0 1px rgba(255,255,255,0.05) inset,0 40px 100px rgba(0,0,0,0.7),0 0 60px rgba(139,92,246,0.15)"}}>
    ...
    <section className="py-16">
      <div className="mx-auto w-full max-w-5xl space-y-8 px-4">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="text-slate-400 mt-4 text-sm tracking-wide md:text-base">
            Everything you need to build fast, secure, and scalable applications.
          </p>
        </div>

        <div className="grid grid-cols-1 divide-x divide-y divide-dashed divide-slate-700/50 border border-dashed border-slate-700/50 rounded-xl overflow-hidden bg-slate-900/30">
          {features.map((feature, i) => (
            <FeatureCard key={i} feature={feature} className="bg-transparent hover:bg-slate-800/40" />
          ))}
        </div>
      </div>
    </section>
...
const Dashboard=({vaults,setPage,setSelectedVault})=>{
  ...
  return(
    <div style={{padding:"40px 48px",maxWidth:1200,margin:"0 auto"}}>
      ...
      {/* NEW: Feature Grid Demo Integrated */}
      <FeatureGridDemo />
          ...
          return(
            <Card key={v.id} className={`fade-up-${i+2}`} style={{cursor:"pointer"}} onClick={()=>{setSelectedVault(v);setPage("Vaults");}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
                <div><div style={{fontWeight:700,fontSize:16,color:"var(--text0)"}}>{v.name}</div><div style={{fontSize:13,color:"var(--text2)",marginTop:4,fontWeight:400}}>{v.description}</div></div>
...
const Vaults=({vaults,setVaults,selected,setSelected})=>{
  ...
  const createVault=()=>{
    ...
    setSaving(true);
    setTimeout(()=>{
      const allocs=form.riskLevel==="aggressive"?[{protocol:"Rivera",pct:35,apy:18.4},{protocol:"Merchant Moe",pct:50,apy:22.1},{protocol:"Agni Finance",pct:15,apy:14.7}]:form.riskLevel==="conservative"?[{protocol:"Rivera",pct:15,apy:18.4},{protocol:"Merchant Moe",pct:10,apy:22.1},{protocol:"Agni Finance",pct:75,apy:14.7}]:[{protocol:"Rivera",pct:33,apy:18.4},{protocol:"Merchant Moe",pct:34,apy:22.1},{protocol:"Agni Finance",pct:33,apy:14.7}];
      const dep=Number(form.deposit);
      const nv={id:Date.now(),name:form.name,description:form.description||"Custom AI vault",riskLevel:form.riskLevel,investmentGoal:form.investmentGoal,status:"active",totalValueUSD:dep,initialDepositUSD:dep,autoRebalanceEnabled:true,allocations:allocs,history:genHistory(7,dep,1.002)};
      setVaults(p=>[...p,nv]);setSelected(nv);setSaving(false);setCreating(false);
    },1200);
  };
  if(creating)return(
    <div style={{padding:"40px 48px",maxWidth:600,margin:"0 auto"}}>
      ...
      <Card className="fade-up-2">
        ...
        <Btn onClick={createVault} disabled={saving||!form.name||!form.deposit} style={{width:"100%",padding:14}}>{saving?"Creating vault...":"Deploy Vault →"}</Btn>
      </Card>
    </div>
  );
  if(selected){
  ...
  return(
    <div style={{padding:"40px 48px",maxWidth:1100,margin:"0 auto"}}>
      <div className="fade-up" style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:32}}>
        <div><h1 style={{fontSize:32,fontWeight:700,letterSpacing:"-0.02em",color:"var(--text0)"}}>Vaults</h1><div style={{fontSize:15,color:"var(--text1)",marginTop:6}}>{vaults.length} active vaults</div></div>
        ...
        <Card className="fade-up-2">
          ...
          {Object.entries(PROTOCOLS).map(([name,p])=>(
            <div key={name} style={{marginBottom:18}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:14,color:p.color,fontWeight:600}}>{name}</span><span style={{fontSize:14,fontFamily:"var(--mono)",fontWeight:700,color:"var(--text0)"}}>{p.apy}% APY</span></div>
              <div style={{height:8,borderRadius:4,background:"rgba(255,255,255,0.05)",overflow:"hidden"}}><div style={{width:`${(p.apy/25)*100}%`,height:"100%",background:`linear-gradient(90deg,${p.color}66,${p.color})`,borderRadius:4}}/></div>
          ...
          <Btn onClick={run} disabled={analyzing} small style={{width:"100%",marginTop:12}}>
            {analyzing?<span style={{display:"flex",alignItems:"center",gap:10,justifyContent:"center"}}><span style={{width:14,height:14,border:"2px solid rgba(255,255,255,0.3)",borderTopColor:"#fff",borderRadius:"50%",display:"inline-block",animation:"spin 0.8s linear infinite"}}/>Analyzing...</span>:"Run AI Analysis ◎"}
          </Btn>
        </Card>
      </div>
            ...
            return(
              <Card key={rec.id} className={`fade-up-${i+1}`} style={{marginBottom:20}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
                  <div><div style={{fontSize:12,color:"var(--text2)",fontFamily:"var(--mono)",marginBottom:6,fontWeight:600}}>VAULT</div><div style={{fontWeight:700,fontSize:16,color:"var(--text0)"}}>{vault?.name}</div></div>
                  <div style={{textAlign:"right"}}><div style={{fontSize:11,color:"var(--text2)",fontFamily:"var(--mono)",fontWeight:600}}>PROJECTED APY</div><div style={{fontSize:28,fontWeight:800,color:"#10b981",fontFamily:"var(--mono)",textShadow:"0 0 20px #10b981"}}>{rec.projectedAPY}%</div></div>
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:20}}>
                  {["Current","Recommended"].map((label,li)=>(
                    <div key={label}>
                      ...
                      {protocols.map((p,pi)=>(<div key={p} style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:13,color:PROTOCOLS[p].color,fontWeight:500}}>{p.split(" ")[0]}</span><span style={{fontSize:13,fontFamily:"var(--mono)",fontWeight:600,color:"var(--text0)"}}>{li===0?rec.current[pi]:rec.recommended[pi]}%</span></div>))}
                    </div>
                  ))}
                </div>
                {rec.status==="pending"?<div style={{display:"flex",gap:12}}><Btn variant="success" onClick={()=>handle(rec.id,"accepted")} small>Accept ✓</Btn><Btn variant="danger" onClick={()=>handle(rec.id,"rejected")} small>Reject ✕</Btn></div>:<Badge label={rec.status==="accepted"?"✓ Accepted":"✕ Rejected"} color={rec.status==="accepted"?"#10b981":"#ef4444"}/>}
              </Card>
            );
          })}
        </div>
...
// ── Settings (Optimized) ───────────────────────────────────────────────────
const Settings=({user,setAuthed})=>(
  <div style={{padding:"40px 48px",maxWidth:640,margin:"0 auto"}}>
    <div className="fade-up" style={{marginBottom:32}}><h1 style={{fontSize:32,fontWeight:700,letterSpacing:"-0.02em",color:"var(--text0)"}}>Settings</h1></div>
    <Card className="fade-up" style={{marginBottom:24}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:20,marginBottom:24}}>
        ...
        <div><div style={{fontWeight:700,fontSize:17,color:"var(--text0)"}}>{user.name}</div><div style={{fontSize:14,color:"var(--text2)"}}>{user.email}</div></div>
      </div>
    ...
    <Card className="fade-up-2" style={{marginBottom:24}}>
      ...
      {[["Auto-rebalance threshold","5% deviation"],["Max slippage tolerance","0.5%"],["AI analysis frequency","Every 6 hours"]].map(([k,v])=>(
        <div key={k} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 0",borderBottom:"1px solid rgba(99,120,200,0.1)"}}>
          <span style={{fontSize:14,color:"var(--text1)",fontWeight:500}}>{k}</span><span style={{fontSize:14,color:"#8b5cf6",fontFamily:"var(--mono)",fontWeight:600}}>{v}</span>
        </div>
      ))}
    </Card>
...
export default function App(){
  ...
  const renderPage=()=>{
    switch(page){
      ...
      case "Settings":  return <Settings user={user} setAuthed={setAuthed}/>;
    }
  };
